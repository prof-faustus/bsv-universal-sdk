...
| `go/` (Go) | REQ-TEST-003 **TS↔Go differential** — an INDEPENDENT Go reimplementation cross-checked byte-for-byte. **Engine:** canonical serialization + domain-separated hashing + debiased `drawValue` + in-between state machine/settlement; a TS corpus (60 vectors / ~630 steps) is replayed and every canonical state hash must match. **Value layer:** the bounded Script interpreter (+ self-contained RIPEMD-160), tx serialization/txid, BIP143 sighash, `verifyTxValue`, `verifyCovenantSpend`, the commit→reveal fairness verification (`verifyBeaconRound`: eligibility/dedup/secret-opens-commit/≥1-honest/seed derivation), AND **ECDSA signature verification via a self-contained Go secp256k1 verifier** (no external dep; low-s enforced) cross-checked against `@noble` (REQ-SEC-001 auth); 160 checks (script / txid / sighash / value / covenant / beacon / auth) must match. Any mismatch fails CI. (Caught a real determinism bug: locale-dependent `localeCompare` → codepoint order, REQ-DET-003.) | `go/diff`, `go/valuediff`, `go/*/*_test.go` |

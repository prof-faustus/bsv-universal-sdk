// Local-practice in-between game (offline, single device). Runs the real engine + crypto. The HUMAN
// chooses every action (REQ-BAN-009 / REQ-CLIENT-001): nothing auto-advances — even dealing is an
// explicit step. Randomness is a genuine commit→reveal beacon round built from local CSPRNG entropy
// and verified before use (REQ-SEC-002/003). Shared by the web and native clients (one source).
...
export function deal(state: InBetweenState): ApplyOutcome {
  const eligible = state.parties.map((p) => hexToBytes(p));
...
function hexToBytes(h: string): Uint8Array {
  if (h.length % 2 !== 0) throw new Error('bad party hex');

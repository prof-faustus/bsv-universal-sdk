...
// Self-contained RIPEMD-160 (no external dependency). Standard reference algorithm; its correctness
// is cross-validated by the differential against @noble/hashes' ripemd160 (OP_HASH160 vectors).
...
func ripemd160Sum(msg []byte) []byte {
	...
	// per-round selection of message word
	rL := [80]uint{
	...
	for off := 0; off < len(padded); off += 64 {
		...
		for j := 0; j < 80; j++ {
			t := rol(al+f(j, bl, cl, dl)+x[rL[j]]+kL[j/16], sL[j]) + el
			al, el, dl, cl, bl = el, dl, rol(cl, 10), bl, t
			jr := 79 - j
			t2 := rol(ar+f(jr, br, cr, dr)+x[rR[j]]+kR[j/16], sR[j]) + er
